...
def affichage_carte():
        ...
        for sub_data in data_stations_map : #boucle qui parcoure toutes les stations
            LATITUDES.append(sub_data["position"]["lat"])
            LONGITUDES.append(sub_data["position"]["lng"])
        data_colors_map = []
...
def func_while_map_open() :
    ...
    text_info_station = [
        ...
        {"nom" : "Banking", "info" : ""},
        {"nom" : "Bonus", "info" : ""},
        {"nom" : "Bike stands", "info" : ""},
        {"nom" : "Available bike stands", "info" : ""},
        {"nom" : "Available bikes", "info" : ""},
        {"nom" : "Status", "info" : ""},
    ...
    while info_map_loop[0] : #page info de la carte
        ...
        if keyboard.is_pressed('space') == True : #on utilise un autre module (keyboard) pour détecter un event clavier même si la fenêtre de pygame n'a pas le focus
            try :
                ...
                hexa_color = "------" #s'il y a un problème pour obtenir la couleur, on enlève les textes des données de station
            if hexa_color not in data_colors_map :
                for info in text_info_station :
                    info["info"] = ""
            else :
                ind_station = data_colors_map.index(hexa_color)
                ind = 0
                for nom in ["name", "contract_name", "address", "banking", "bonus", "bike_stands", "available_bike_stands", "available_bikes", "status"] :
                    text_info_station[ind]["info"] = data_stations_map[ind_station][nom]
                    ind+=1
                timestamp = datetime.datetime.fromtimestamp(data_stations_map[ind_station]["last_update"]//1000)
                text_info_station[9]["info"] = timestamp.strftime('%d-%m-%Y %H:%M:%S')
        ...
        if while_loading_map[0] and ind_loading_anim[0]%6 == 0:
            ...
            text_surf = font2.render(f"Chargement{['.', '..', '...'][ind_loading_anim[1]]}", True, (255,255,255))
            ...
            pygame.draw.rect(window, (155,155,155), pygame.Rect(conv_sizex(50), conv_sizey(220), size_screen[0]-conv_sizex(100), size_screen[1]-conv_sizey(300)), 0,3)
            ...
            window.blit(text_surf, text_surf.get_rect(center = (size_screen[0]//2, conv_sizey(240))))
            text_surf = font2.render(f"placez votre sourie sur un point, puis appuyez sur la touche espace", True, (255,255,255))
            window.blit(text_surf, text_surf.get_rect(center = (size_screen[0]//2, conv_sizey(260))))
